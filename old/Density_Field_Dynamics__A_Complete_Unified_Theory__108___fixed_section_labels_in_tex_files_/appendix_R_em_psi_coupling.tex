...
phenomenological parameter $\kappa$ controls the \textit{differential} 
...
\subsubsection{Bounds on $\kappa$}

\paragraph{Accidental bound from cavity stability.}
Absence of $2\omega$ parametric instabilities in extreme-Q resonators constrains unintended EM$\leftrightarrow\psi$ pumping. This provides headroom consistent with $|\kappa| \lesssim 1$.

\paragraph{LPI residual as $\kappa$ constraint.}
...
\end{align}
where $K_\epsilon^{(S)}$ is the atomic EM-energy sensitivity.  A future high-precision cavity--atom measurement would constrain $\kappa$ through this residual channel.
...
The $\kappa$ parameter can be measured via:
\begin{enumerate}
...
\item Constrained by cavity stability: $|\kappa| \lesssim 1$
\item Directly measurable via sector-resolved LPI slope
\end{itemize}
\textbf{Falsification:} If TE/TM cavity comparisons show no $\psi$-dependent split at $10^{-5}$ precision, $\kappa \approx 0$ is confirmed.
\end{tcolorbox}

...
The TT sector propagates at $c$ exactly---a structural result proven from $O(3)$ irreducible decomposition, not fine-tuning (\S\ref{sec:gw-ct-proof}). Within the $\mathbb{C}P^2 \times S^3$ spectral completion, both $\psi$ and $h_{ij}^{\rm TT}$ are derived as irreducible components of the same zero-mode parent tensor on the internal manifold (\S\ref{sec:gw-spectral-origin}). A Lichnerowicz rigidity analysis proves no unwanted massless modes arise; the single scalar modulus is determined by the $\alpha$--$G$ constraints at the Einstein product condition $R_2/R_1 = 1/\sqrt{3}$ and decouples at Planck mass (Appendix~\ref{app:O_alpha57_clean}). The theory carries two tensor polarizations and satisfies the standard quadrupole formula (\S\ref{sec:gw}). Binary pulsar orbital decay agrees at 0.2\%. LIGO/Virgo observations are consistent.
...
\item G--H$_0$ invariant: $(H_0/M_P)^2 = \alpha^{57}$ spectral-action-derived (Appendix~\ref{app:O_alpha57_clean})

...
%-----------------------------------------------------------------------------
\subsection{Cooper-Pair Mass Anomaly from $A_5$ Pair Space}\label{app:cooper-pair}
%-----------------------------------------------------------------------------

The Cooper-pair mass in niobium, measured by Tate \textit{et al.}
(1989) via the London moment, exceeds $2m_e$ by
$\delta = 92 \pm 21$~ppm---a $4.4\sigma$ anomaly unexplained for
36~years~\cite{AlcockCooperPair2026}.  Within the $A_5$ microsector, each electron's generation
quantum number lives in the fundamental $V^*$ ($\dim V^* = 3$).
The pair tensor product decomposes as
$V^* \otimes V^* = S^2(V^*) \oplus \Lambda^2(V^*)$ with
$S^2(V^*) = \mathbf{1} \oplus \mathbf{5}$ and
$\Lambda^2(V^*) = \mathbf{3}$.

Two pairing-symmetry selection rules follow:
\begin{enumerate}
\item \textbf{Angular cancellation:} the quintet $\mathbf{5}$
  exchange channel couples maximally to $s$-wave condensates
  (isotropic gap) but vanishes for $d$-wave condensates
  ($\int_0^{2\pi} \cos 2\phi\,d\phi/(2\pi) = 0$).
\item \textbf{Representation orthogonality:} spin-triplet ($p$-wave)
  pairs live in $\Lambda^2(V^*) = \mathbf{3}$, orthogonal to the
  quintet by $A_5$ representation theory alone.
\end{enumerate}
The conjectured coefficient is
$\delta = \sqrt{3}\,\alpha^2 = 92.23$~ppm (0.01$\sigma$ match),
with $\sqrt{3} = \sqrt{N_{\rm gen}}$ from incoherent amplitude
addition of three generation channels and $\alpha^2$ from the
$\psi$--EM vertex structure.  This prediction is universal for
$s$-wave superconductors and zero for $d$-wave and $p$-wave
materials---a distinction testable by multi-material London-moment
measurements at $\leq 20$~ppm precision.

%-----------------------------------------------------------------------------
\subsection{EM--Gravity Cross-Term: Gravitational Weight Anomaly}\label{app:weight-anomaly}
%-----------------------------------------------------------------------------

The DFD stress tensor contains a cross term between $\psi_{\rm grav}$
and the above-threshold EM contribution $\delta\psi_{\rm EM} =
\kappa_G(\eta - \eta_c)\Theta$, yielding a fractional weight anomaly
for a device of mass $m$ carrying EM energy $U_{\rm EM}$ above
threshold:
\begin{equation}\label{eq:weight-anomaly}
\frac{\Delta w}{w} = \kappa_G \cdot \frac{U_{\rm EM}}{mc^2}
= \frac{3}{8\alpha} \cdot \frac{U_{\rm EM}}{mc^2}\,.
\end{equation}
For a 10\,T superconducting magnet ($U_{\rm EM} = 40$\,kJ,
$m = 10$\,kg): $\Delta w/w = 2.3 \times 10^{-12}$.
Next-generation atom-interferometric gravimeters approach
$10^{-12}$--$10^{-13}$, placing this prediction at the edge of
sensitivity.  The signature is distinctive: the effect scales as
$B^2 V/(2\mu_0 mc^2) \times 3/(8\alpha)$, with the $\alpha$-dependence
as the smoking gun.  A null control at $\eta < \eta_c$ (higher
ambient pressure) eliminates conventional systematics.
...
\textbf{Cooper-Pair Mass Anomaly (\S\ref{app:cooper-pair}):}
\begin{itemize}[nosep]
\item Prediction: $\delta = \sqrt{3}\,\alpha^2 = 92.23$\,ppm (universal for $s$-wave)
\item Two selection rules: $d$-wave $\to$ 0 (angular cancellation), $p$-wave $\to$ 0 (representation orthogonality)
\item Test: multi-material London-moment measurement at $\leq 20$\,ppm
\end{itemize}

\textbf{EM--Gravity Weight Anomaly (\S\ref{app:weight-anomaly}):}
\begin{itemize}[nosep]
\item Prediction: $\Delta w/w = (3/8\alpha) \cdot U_{\rm EM}/(mc^2) \approx 2.3\times10^{-12}$ for 10\,T magnet
\item Test: next-generation atom gravimeters at $10^{-12}$--$10^{-13}$
\end{itemize}

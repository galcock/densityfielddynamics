...
% v4: kappa = alpha/4 derived from gauge-emergence correction (no longer free)
% Based on v3 + Kappa_From_Action derivation
% March 2026
%=============================================================================
...
medium with vacuum force scale $K_0 = c^4/(8\pi G)$.  The dimensionless
...
curves.  The vacuum stiffness $K_0 = \hbar H_0^2/(8\pi\alpha^{57}c)$
is parameter-free given the master invariant and one measured scale.
The constitutive split parameter $\kappa = \alpha/4 \approx 1.82 \times 10^{-3}$
is derived from the gauge-emergence correction to the optical metric,
connecting the E/B asymmetry to the same physics that determines the
EM--$\psi$ coupling threshold.
...
\item The vacuum resists deformation with a force scale (stiffness)
$K_0 = c^4/(8\pi G)$, the same coefficient that appears in the
Einstein field equations.  The energy density of the loaded vacuum is
$u_\psi = K_0\,|\nabla\psi|^2$.
...
through $K_0 = \hbar H_0^2/(8\pi\alpha^{57}c)$.
\end{enumerate}

The paper is organized as follows.  Section~\ref{sec:stiffness}
defines and derives the vacuum stiffness $K_0$.
...
%=============================================================================
\section{The Vacuum Stiffness $K_0$}
\label{sec:stiffness}
%=============================================================================

\subsection{Definition}

The vacuum stiffness $K_0$ is a force scale (units of newtons) that
converts the dimensionless $\psi$-field gradient into physical energy
density:
\begin{equation}
u_\psi = K_0\,|\nabla\psi|^2, \qquad
\boxed{K_0 \equiv \frac{c^4}{8\pi G} \approx 4.82 \times 10^{42}~\text{N}.}
\label{eq:K0-def}
\end{equation}
Since $[K_0] = \text{N}$ and $[|\nabla\psi|^2] = \text{m}^{-2}$,
the energy density $u_\psi$ has the correct units
$\text{N}\cdot\text{m}^{-2} = \text{J/m}^3$.
...
physical interpretation: $K_0$ is the \emph{vacuum force scale}
(stiffness) of the vacuum treated as an electromagnetic storage
medium.  Equivalently, $G = c^4/(8\pi K_0)$.
...
\end{equation}
Given the master invariant and one measured scale ($H_0$), $K_0$ is
parameter-free:
\begin{equation}
K_0 = \frac{c^4}{8\pi G}
= \frac{\hbar H_0^2}{8\pi\,\alpha^{57}\,c}.
\label{eq:K0-from-invariant}
\end{equation}
This expresses $K_0$ entirely in terms of $(\hbar, c, H_0, \alpha)$
...
\label{eq:K0-planck}
\end{equation}
where $F_P = c^4/G$ is the Planck force.
...
topology: a 1\% shift in $\alpha^{-1}$ produces a 57\% shift in $K_0$.
...
The constitutive split parameter $\kappa$ is determined by the
gauge-emergence correction to the optical metric.  At tree level, the
Gordon optical metric gives $\kappa = 0$ (symmetric E/B response).  The
gauge-emergence auxiliary metric introduces a correction proportional to
the effective coupling $\alpha_{\rm eff} = \alpha/n_2^2 = \alpha/4$
(where $n_2 = 2$ is the SU(2) frame stiffness), giving:
\begin{equation}
\boxed{\kappa = \frac{\alpha}{4} \approx 1.82 \times 10^{-3}.}
\label{eq:kappa-derived}
\end{equation}
This is the same value as the EM--$\psi$ nonlinear coupling threshold
$\eta_c$ (Section~\ref{sec:em-coupling}), confirming that both arise
from the same gauge-emergence physics.
...
4.2\times 10^{-6}$.  With $\kappa = \alpha/4 \approx 1.82 \times 10^{-3}$,
the correction beyond the $\kappa = 0$ baseline is at the $\sim 10^{-3}\psi$
level.  The LPI violation parameter is
...
vertex, as derived from the gauge--$\psi$ Lagrangian coupling.  Note
that $\eta_c = \kappa$: both the constitutive split and the EM
back-reaction threshold are controlled by the same gauge-emergence
coupling $\alpha/4$.
...
and the restoring force scale is constant: $K = c^4/(4\pi G) = 2K_0$.  The
...
\label{eq:stress-def}
\end{align}
where $a_* = 2a_0/c^2$ is the characteristic inverse-length scale
(with $a_0$ the crossover acceleration) and
...
$u = \frac{1}{2}K s^2$, confirming $K = K_0 = c^4/(8\pi G)$.
Since $[K_0] = \text{N}$ and $[|\nabla\psi|^2] = \text{m}^{-2}$,
we have $[u_\psi] = \text{N/m}^2 = \text{J/m}^3$ as required.
...
\label{eq:strain}
\end{equation}
where $a_* = 2a_0/c^2 \approx 2.67 \times 10^{-27}$~m$^{-1}$ is the
characteristic inverse-length scale and
$a_0 = 2\sqrt{\alpha}\,cH_0$ is the crossover acceleration.
Note that $a_*$ has units of m$^{-1}$ (not m/s$^2$), so the strain
$s = |\nabla\psi|/a_*$ is indeed dimensionless.
...
\label{eq:stress}
\end{equation}
with dimensions $[\sigma] = \text{N}\cdot\text{m}^{-1}
= \text{J/m}^2$, so that
$[\nabla\cdot\bm{\sigma}] = \text{N/m}^2 = \text{Pa}$,
as verified in Appendix~\ref{app:dimensions}.
...
$K_0$) is determined by vacuum mode structure.
...
distinction between $K_0$ (vacuum force scale) and $\rho_\Lambda$
...
\begin{tabular}{lcc}
\toprule
\textbf{Scale} & \textbf{Value} & \textbf{Role} \\
\midrule
QFT cutoff $\rho_P c^2$ & $\sim 10^{113}$~J/m$^3$ & Naive mode-sum \\
Stiffness $K_0$ & $\sim 10^{42}$~N & Vacuum force scale \\
Dark energy $\rho_\Lambda c^2$ & $\sim 10^{-9}$~J/m$^3$ & Residual strain \\
\bottomrule
\end{tabular}
\end{center}

The critical distinction: $K_0$ is the vacuum force scale (resistance to
deformation), not an energy density.  The observed
...
\alpha^{57}$~\cite{Alcock2025AbInitio} and one measured scale, $K_0$
is parameter-free:
$K_0 = \hbar H_0^2/(8\pi\alpha^{57}c)$.  Whether this expression
admits an independent derivation from the microsector topology remains
open.

\subsection{The constitutive split $\kappa = \alpha/4$}

The constitutive split $\kappa = \alpha/4$ is derived, not fitted,
connecting the E/B asymmetry to the same gauge-emergence physics that
determines the EM--$\psi$ coupling threshold.  The derivation proceeds
in two steps: (i) the Gordon optical metric forces $\kappa = 0$ at
tree level (symmetric E/B response); (ii) the gauge-emergence auxiliary
metric, which encodes the SU(2) frame stiffness $n_2 = 2$, introduces
a correction $\alpha_{\rm eff} = \alpha/n_2^2 = \alpha/4$.  The
coincidence $\kappa = \eta_c$ (the EM back-reaction threshold) is not
accidental---both measure the strength of the gauge-$\psi$ coupling
beyond the tree-level optical metric.
...
\item The constitutive split is derived:
$\kappa = \alpha/4 \approx 1.82 \times 10^{-3}$.
...
\item LPI slope $\xi(\kappa)$: species-dependent with
$\kappa = \alpha/4 \approx 1.82 \times 10^{-3}$.
\item TE/TM cavity asymmetry: $\sim 2\kappa\psi \sim 10^{-3}\psi$
frequency splitting, giving $\Delta\nu/\nu \sim 10^{-3}\psi
\sim 10^{-9}$ near Earth's surface ($\psi_\oplus \sim 10^{-6}$).
...
\item Time dependence of $K_0$: if $G(t)$ varies with cosmic epoch,
...
The vacuum stiffness $K_0 = \hbar H_0^2/(8\pi\alpha^{57}c)$ is
determined by topology plus one scale measurement.  The constitutive
split parameter $\kappa = \alpha/4 \approx 1.82 \times 10^{-3}$ is
derived from the gauge-emergence correction to the optical metric,
connecting the E/B asymmetry to the EM--$\psi$ coupling threshold.
The nonlinear regime admits a continuum-mechanics interpretation in
which the vacuum exhibits strain stiffening at low gradients---the
mechanism behind flat rotation curves.  The framework provides a
natural understanding of the vacuum energy hierarchy consistent with
the $\alpha^{57}$ suppression, and makes falsifiable predictions for
the SQMS Q-ratio test, TE/TM cavity splitting at the $\sim 10^{-3}\psi$
level, and cavity--atom comparisons.
...
$= \mathrm{N\,m^{-1}}$
$= \mathrm{J/m^2}$ & \checkmark \\
\addlinespace
$\nabla\!\cdot\!\bm{\sigma}$ &
$\mathrm{m^{-1}}\cdot\mathrm{N\,m^{-1}}
= \mathrm{N/m^2} = \mathrm{Pa}$ & \checkmark \\
\addlinespace
Field eq.\ LHS:
$\nabla\!\cdot\!(\mu\nabla\psi)$ &

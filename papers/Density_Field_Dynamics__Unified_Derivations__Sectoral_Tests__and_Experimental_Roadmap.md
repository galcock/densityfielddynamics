...
All massive species experience the same ψ-derived acceleration a = (c2 /2)∇ψ, ensuring universality of free fall and preventing composition-dependent forces.
...
duality deformation via eψ dt/a, and (iii) reduced ISW/growth at low k. Implementation
...
−5
sifier: ∆ϕNR = ψ ds, achromatic under dual-wavelength suppression at 10 -rad
sensitivity.
...
amplitude (−1.0 ± 0.3 × 10−17 ) is phase-aligned but statistically insignificant after control
...
R ψ
cancellation of dispersion; (iv) add Geff (a) and DL ∝ e dt/a hooks to existing cosmology
pipelines [52, 40, 34].
...
For completeness,
the full 2PN evaluation can be written explicitly. Using ψ = rs /r
√
2
2
and r = b + z , the transverse gradient ∇⊥ ψ = (rs b/r3 ) b̂ gives the first-order deflection
α(1) = 2rs /b. The quadratic term in ln n = ψ − 21 ψ 2 yields
Z +∞
πr2
(2)
ψ ∂b ψ dz = 2s ,
...
−∞
Rz
and the path (Born) correction from the perturbed trajectory δb(z) = − −∞ α(1) (z ′ )dz ′ —a
standard result for rays in inhomogeneous media [14]—gives
(2)
...
16b
b
or equivalently α = 4ε + (15π/4)ε2 with ε = GM/(c2 b). This step-by-step evaluation
confirms that the graded-index form n = eψ reproduces the GR 2PN coefficient without
...
α=
∇⊥ ln n dz =
∇⊥ ψ − 21 ψ 2 + O(ψ 3 ) dz + path correction.
(12)
−∞
...
DFD predicts ξ = +1, GR gives ξ = 0 [6, 5]. This ξ = 1 prediction applies primarily to
cross-sector ratios (photon–atom or ion–neutral). Same-sector comparisons (atom–atom,
resonator–resonator) ideally cancel the sectoral term, giving ξ ≃ 0 for co-located nondispersive references; however, small residual slopes can arise from cavity dispersion, effective length change, or true GR redshift differences between non-co-located laboratories.
These effects are already bounded at |αw |, |αLM | ≲ 10−8 (Secs. 3.3, 3.4), well below the
10−15 target precision.
...
Example: using tabulated polarizabilities αe (0) = 2.2 × 10−39 J m2 V−2 for Sr and
2.4×10−39 J m2 V−2 for Yb+ gives Kion ≈ 1.4×10−3 and Kneutral ≈ 0, predicting a fractional
slope near 10−17 for ∆Φ/c2 ≈ 10−14 —matching the observed ROCIT amplitude.
...
than generic seasonal effects. The driver phase was fixed a priori by Earth’s perihelion,
...
σA ≈ 1.7 × 10−18 , establishing statistical robustness [46, 45].
...
Tully–Fisher scaling (Sec. 7) [63, 58, 59].
...
dynamics [Eqs. (69)–(70)] reveals a unified refractive correction to phase evolution and
...
AION) yield ∆ϕDFD = −(mg∆hT /ℏ) identical to Eq. (70). By modulating launch height
...
ψ field equation. Linearizing the quasi-static DFD equation (3) about a homogeneous
